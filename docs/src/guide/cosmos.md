...
uight's fixture format is compatible with react-cosmos 7.x fixture files, so the
...
package you are about to uninstall. `uight init` does all three.
...
react-cosmos found: cosmos.config.json; react-cosmos in package.json
  ...
  ✓ uight.config.json  rootDir → fixturesDir: src
  ↦ src/__fixtures__/button.fixture.tsx  from src/__fixtures__/button.tsx — uight finds
    fixtures by name, not by directory; react-cosmos → uight (useValue → useFixtureInput)
...
**The config.** `cosmos.config.json` becomes `uight.config.json`, key by key:
...
file inside a `__fixtures__/` directory. uight only does the first. So
...
**The imports.** `react-cosmos/client` becomes `uight`:
...
`cosmos.decorator.tsx` needs no change at all — uight recognizes both that name and
`uight.decorator.tsx`.

## The report on its own

```bash
bunx uight cosmos          # a summary
bunx uight cosmos --json   # per file, for CI
```
...
uight is an independent implementation and is not affiliated with or endorsed by the
