...
a static JSON file under `/r/`, copied out of `packages/uaight/registry` by
...
| URL                                     | What it is                                      |
| --------------------------------------- | ----------------------------------------------- |
| `https://uaight.dev/r/registry.json`    | The registry index — every item, no file bodies |
| `https://uaight.dev/r/{name}.json`      | An item, tracking the latest minor              |
| `https://uaight.dev/r/v0.0/{name}.json` | The same item, pinned to a minor                |
...
{
	"registries": {
		"@uaight": "https://uaight.dev/r/{name}.json",
		"@uaight-v0-0": "https://uaight.dev/r/v0.0/{name}.json"
	}
}
```

```bash
npx shadcn add @uaight/control-panel        # tracks the latest minor
npx shadcn add @uaight-v0-0/control-panel   # pinned to v0.0
```

The namespace form is the better one: items that depend on other items — `control-panel`
pulls in `control-panel-inputs` — reference them as `@uaight/…` (or `@uaight-vX-Y/…` on
...
| `../../shared/types.ts`, `../chrome-context.ts`, `../../shared/wire.ts`, `../../shared/fixture-id.ts`, `../../runtime/codec-editors.tsx` | `uaight/chrome` — the frozen surface (§11.4)                                   |
...
- The real `shadcn` CLI (4.16.1) has resolved and installed these items over HTTP, from
  this site's built `public/` served at a URL root — the flat `@uaight/{name}` form, the
  direct-URL form, the pinned `@uaight-v0-0/{name}` form, and registry dependencies
  followed from one item to another (including from a pinned item to a pinned item).
  Files land in `components/` and `styles/uaight-chrome.css`, and `uaight` is added to
  `dependencies`.
- **The installed files compile.** In a scratch project with a stock `tsconfig.json` —
  ...
  over all thirteen files that `@uaight/fixture-tree`, `@uaight/control-panel`,
  `@uaight/prop-table`, `@uaight/command-palette`, `@uaight/viewport-toolbar` and the
  pinned `@uaight-v0-0/*` items install, typechecked against the real `uaight/chrome`
  declarations. This is the claim §11.1 asks for; it was false until the specifiers were
  rewritten at emit time.
- What that does **not** prove is `https://uaight.dev` itself. The domain has to be
