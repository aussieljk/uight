	...
	return (
		<div className="flex h-full min-h-0 flex-col">
			<div className="flex h-9 shrink-0 items-center gap-2 border-b border-[var(--u-line)] px-3">
				<h2 className="text-[12px] font-medium text-[var(--u-fg)]">Controls</h2>
				...
				{edited ? (
					<button
						...
						className={cx(
							"ml-auto h-6 rounded-sm px-1.5 text-[11px] text-[var(--u-accent)] hover:bg-[var(--u-bg-hover)]",
							FOCUS_RING,
							MOTION,
						)}
					>
			...
			{droppedPatches > 0 ? (
				...
				<p
					role="status"
					className="border-b border-[var(--u-line)] bg-[var(--u-danger-soft)] px-3 py-1.5 text-[11px] leading-4 text-[var(--u-danger)]"
				>
					{droppedPatches} {droppedPatches === 1 ? "setting" : "settings"} no longer apply.
				</p>
			) : null}
