/**
 * One command from a react-cosmos repository to a working `/uaight`.
 *
...
/** Modules whose imports move to `uaight`. */
...
/**
 * Cosmos hook → uaight hook. The right-hand side is exported from `uaight`
...
/**
 * Named imports uaight has no equivalent for. They are left in place, not
 * rewritten to something that would compile and mean something else.
 */
const DECLINED_IMPORTS: Record<string, string> = {
	useFixtureState: "no equivalent — uaight has no fixture-state protocol",
	setFixtureState: "no equivalent — uaight has no fixture-state protocol",
	useCosmosConfig: "no equivalent — read `uaight.config.json` yourself",
	RendererProvider: "use the preview entry (§6.4) instead",
	FixtureLoader: "internal to cosmos — uaight loads fixtures itself",
};
...
export interface CosmosTranslation {
	/** The `uaight.config.json` body, empty when every default already fits. */
 ...
 * owns all of these once uaight is a plugin, so dropping them is the answer,
...
const SERVER_KEYS: Record<string, string> = {
	...
	exportPath: "use `uaight build --out <dir>`",
	watchDirs: "Vite watches the project already",
	webpack: "uaight is a Vite plugin",
	vite: "uaight is a Vite plugin — this config is your vite.config now",
	plugins:
...
/**
 * Translate a parsed `cosmos.config.json` into uaight plugin options.
 *
 * The one translation worth explaining is `fixturesDir`. Cosmos means "a
 * directory *name* that marks fixtures anywhere in the tree"; uaight's
...
export function translateCosmosConfig(json: Record<string, unknown>): CosmosTranslation {
	const options: UaightPluginOptions = {};
...
export interface ImportRewrite {
	source: string;
	changed: boolean;
	/** `cosmosName → uaightName`, for the transcript. */
...
/**
 * Point a fixture's cosmos imports at `uaight`.
 *
 ...
 * uaight declines keeps its original module: the file then imports from both
...
export function rewriteCosmosImports(source: string, filename: string): ImportRewrite {
	...
	for (const statement of (program.body ?? []) as unknown[]) {
		...
		for (const specifier of specifiers) {
			...
			if (name !== null) {
				declined[name] = DECLINED_IMPORTS[name] ?? "no equivalent in uaight";
			} else {
				declined[specifier.type === "ImportDefaultSpecifier" ? "default" : "namespace"] =
					"uaight has no default or namespace export to move this to";
			}
		}
		...
		const clause = moving
			...
			.join(", ");
		const moved = `import { ${clause} } from "uaight";`;
...
/**
 * Cosmos's directory convention, expressed as uaight's suffix convention.
 *
 ...
 * it. Exported from `uaight/vite` so it can run in CI.
 */
...
export function formatCosmosReport(report: CosmosReport): string {
	...
	if (report.renames.length) {
		lines.push(
			`${report.renames.length} need renaming to carry the suffix — ` +
				"uaight finds fixtures by name, not by directory",
		);
	}
	...
	if (report.translation) {
		...
		if (!translated.length && !Object.keys(dropped).length) {
			lines.push("  · every key is already a uaight default");
		}
	}
