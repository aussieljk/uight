...
const PLUGIN_CALL = "uaight({ storybook: true })";
const PLUGIN_IMPORT = 'import { uaight } from "uaight/vite";';
 ...
 */
export function addUaightToViteConfig(source: string, filename: string): ConfigEdit {
	if (/["']uaight\/vite["']/.test(source)) {
	...
	// Prepending rather than appending: uaight only contributes virtual modules
...
/**
 * Add `uaight` to `devDependencies`, preserving the file's own formatting.
 *
...
export async function migrateProject(options: MigrateOptions): Promise<MigrationResult> {
	...
	if (!pkg) {
		changes.push({
			path: "package.json",
			action: "skip",
			detail: "not found or not valid JSON — add `uaight` to devDependencies yourself",
		});
	} else {
		...
		if (next === null) {
			...
			changes.push({
				path: "package.json",
				action: "edit",
				detail: `uaight@${version} → devDependencies`,
				contents: next,
			});
	...
	if (result.report && (result.report.files > 0 || result.evidence.length > 0)) {
		...
		lines.push(
			`${plural(files, "CSF file")}, ${plural(stories, "story", "stories")} — ` +
				`${clean} use nothing uaight declines`,
		);
