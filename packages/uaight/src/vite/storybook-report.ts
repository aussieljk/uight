 ...
 * evaluating uaight actually asks: *how much of our corpus works?*
 *
...
export function formatStorybookReport(report: StorybookReport): string {
	...
	lines.push(
		`${report.files} CSF files, ${report.stories} stories — ` +
			`${report.clean} files use nothing uaight declines`,
	);
	lines.push(
		report.preview
...
export function reportPath(cfg: ResolvedUaightConfig, file: string): string {
