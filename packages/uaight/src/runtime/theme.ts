 ...
 *
 * Deliberately not a fixture hook. `useUaightTheme` needs no fixture runtime,
 * because the reader is the provider tree *above* every fixture.
 */

import { useSyncExternalStore } from "react";
import { THEME_ATTRIBUTE } from "../shared/types.ts";
import type { ResolvedUaightTheme } from "../shared/types.ts";
 ...
 * on the default.
 */
export function subscribeUaightTheme(listener: () => void, doc?: Document): () => void {
	const target = doc ?? (typeof document === "undefined" ? undefined : document);
...
export function useUaightTheme(): ResolvedUaightTheme {
