...
function layoutStyle(layout: FixtureMeta["layout"], inOverview = false): React.CSSProperties {
	...
	if (layout === "centered") {
		return {
			// `100%` cannot resolve here: `#uaight-root` sets only `min-height`, so
...
export function RendererApp(props: RendererAppProps): React.ReactElement {
	...
	const mismatch = React.useMemo<RendererError | null>(() => {
		...
		if (config.version !== UAIGHT_VERSION) {
			return {
				kind: "protocol",
				message: `version mismatch: plugin uaight ${config.version}, runtime uaight ${UAIGHT_VERSION}. One of them is a stale build artefact.`,
			};
		}
		return null;
