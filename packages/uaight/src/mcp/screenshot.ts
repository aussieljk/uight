 ...
 *    that *is* evidence is `#uaight-root` inside the frame document having
...
export const PLAYWRIGHT_MISSING_MESSAGE =
	...
	`optional dependency so that an install of uaight does not pay for three browser ` +
...
export interface RenderOptions {
	/** Explorer base URL, already discovered (`/@uaight/health`). No port here. */
	base: string;
	/** The explorer route from `/@uaight/config.json`, e.g. `/uaight`. */
	route: string;
	...
	/** Resolved theme (`data-uaight-theme`). Omit to take the browser default. */
	theme?: "light" | "dark";
	/** Capture the whole explorer chrome instead of just the fixture frame. */
	fullPage?: boolean;
	/** How long to wait for `#uaight-root` to have children. */
	timeout?: number;
...
const FRAME_RENDERED = `() => {
	const frame = document.querySelector('iframe[data-uaight-frame]');
	const root = frame && frame.contentDocument
		? frame.contentDocument.getElementById('uaight-root')
		: null;
...
export async function renderFixture(options: RenderOptions): Promise<RenderResult> {
	...
	try {
		...
		await page.waitForSelector("iframe[data-uaight-frame]", { timeout });
		await page.waitForFunction(FRAME_RENDERED, undefined, { timeout, polling: 100 });

		const bytes = fullPage
			? await page.screenshot({ type: "png", fullPage: true })
			: await page.locator("iframe[data-uaight-frame]").screenshot({ type: "png" });
