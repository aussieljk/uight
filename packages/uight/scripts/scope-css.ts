 ...
 * matches inside `.uight-root`. That is what makes the packaged CSS immune to
...
export const SCOPE_SELECTOR = ".uight-root";
...
export interface ScopeOptions {
	 ...
	 * *under* a `.uight-root` ancestor, so a plain descendant combinator would
	 * be sufficient. But the natural thing for a caller to write is
	 * `<div className="uight-root flex flex-col">`, and under a strict
...
export function scopeSelector(selector: string, options: ScopeOptions = {}): string[] {
	...
	// Already confined — hand-written rules in uight.css spell the scope out
