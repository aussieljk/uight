...
export const UightChromeContext = createContext<UightChromeApiV1 | null>(null);

export function useUightChrome(): UightChromeApiV1 {
	const api = useContext(UightChromeContext);
	if (!api) {
		throw new Error(
			"[uight] useUightChrome() was called outside <Uight />. Chrome components " +
				"read the explorer's state from the mount that renders them, so they have to " +
				"be rendered by one — pass them through <Uight components={{ … }} /> (or " +
				"UightProvider) rather than mounting them yourself.",
		);
	}
	return api;
}
