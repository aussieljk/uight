 ...
 * CSS, and would escape the `.uight-root` scoping contract.
 */

import { UIGHT_CSS } from "../styles/generated.ts";
...
export function ensureStyles(doc: Document, nonce?: string | undefined): void {
	...
	el.setAttribute(STYLE_MARKER, UIGHT_VERSION);
	if (nonce) {
		el.setAttribute("nonce", nonce);
		el.nonce = nonce;
	}
	el.textContent = UIGHT_CSS;
...
export function styleTag(nonce?: string | undefined): string {
	...
	const css = UIGHT_CSS.replace(/<\/style/gi, "<\\/style");
	return `<style ${STYLE_MARKER}="${UIGHT_VERSION}"${attr}>${css}</style>`;
}
