 ...
 *
 * Both halves keep the same `uight:…:<route>:<mountId>` namespacing, so two
...
/**
 * Keyed by route AND mount: two `<Uight />` mounts on one page are two
 * explorers with two selections (§5.4 makes the same distinction for the URL,
 * via `routerId`), and the same mount on `/uight` and on `/design/buttons` is
 * two different places to be.
 */
export function sessionKey(route: string, mountId: string): string {
	return `uight:session:${route}:${mountId}`;
}
...
export function prefsKey(sessionStorageKey: string): string {
	return sessionStorageKey.replace(/^uight:session:/, "uight:prefs:");
}
