...
export const FRAME_ROOT_ID = "uight-root";

/**
 * A `.uight-root` sibling written into the frame document. Renderer-side
 ...
 */
export const FRAME_CHROME_ID = "uight-frame-chrome";

/** Marks the injected `<style>` so we inject once per document (§10.3). */
export const STYLE_MARKER = "data-uight-styles";

/** Every element we render lives under this class; the compiled CSS is scoped to it. */
export const ROOT_CLASS = "uight-root";

/**
 * The class pair every uight mount point carries: our scope class and ljkui's
 * theme-root class. The chrome is built from ljkui components, so its
 * stylesheet — scoped alongside ours (§10.3) — expects `ljkui` on the same
 * element as `ROOT_CLASS`. React mounts get it from `<Theme>`; the frame and
 * static documents are written as HTML and have to spell it out.
 */
export const ROOT_CLASSES = `${ROOT_CLASS} ljkui`;
