...
function channel<T extends object>(message: T): T & { __uight: typeof CHANNEL } {
	return { __uight: CHANNEL, ...message };
}
...
export function createRendererChildTransport(
		...
		if (!SUPPORTED_PROTOCOL_VERSIONS.includes(init.protocolVersion)) {
			...
			options.onMismatch?.({
				...
				message:
					...
					`this renderer (uight ${UIGHT_VERSION}) supports ${SUPPORTED_PROTOCOL_VERSIONS.join(", ")}`,
			});
			return;
		}
	...
	function onMessage(event: MessageEvent): void {
		...
		if (!mountId || envelope.mountId !== mountId) {
			...
			if (mismatches === MOUNT_MISMATCH_LIMIT) {
				...
				console.error(
					`[uight] ${mismatches} messages dropped for a mismatched mountId — two mounts sharing a frame?`,
				);
			}
			return;
		}
		...
		if (inboundSequence >= 0 && envelope.sequence !== inboundSequence + 1) {
			// eslint-disable-next-line no-console
			console.warn(
				`[uight] host message sequence gap: expected ${inboundSequence + 1}, saw ${envelope.sequence}`,
			);
		}
	...
	if (!parent || parent === win) {
		options.onMismatch?.({
			kind: "bootstrap",
			message: "the uight renderer was loaded outside a frame; there is no host to talk to",
		});
	} else {
		sendReady();
		void readySentAt;
	...
	function handleReady(versions: readonly number[], rendererVersion: string): void {
		...
		if (version === null) {
			...
			setStatus("error", {
				kind: "protocol",
				message:
					`no mutually supported protocol version. Host (uight ${options.pluginVersion ?? UIGHT_VERSION}) ` +
					`supports ${SUPPORTED_PROTOCOL_VERSIONS.join(", ")}; renderer (uight ${rendererVersion}) offers ${versions.join(", ")}.`,
			});
			return;
		}
	...
	function onMessage(event: MessageEvent): void {
		...
		if (envelope.mountId !== mountId) {
			...
			if (dropped === MOUNT_MISMATCH_LIMIT) {
				// eslint-disable-next-line no-console
				console.error(
					`[uight] ${dropped} frame messages dropped for a mismatched mountId (${envelope.mountId} ≠ ${mountId})`,
				);
			}
			return;
		}
		...
		if (inboundSequence >= 0 && envelope.sequence !== inboundSequence + 1) {
			// eslint-disable-next-line no-console
			console.warn(
				`[uight] renderer message sequence gap: expected ${inboundSequence + 1}, saw ${envelope.sequence}`,
			);
		}
		inboundSequence = envelope.sequence;
