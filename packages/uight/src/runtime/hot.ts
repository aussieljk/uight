 ...
 * `virtual:uight/runtime` — which both realms import, and which accepts
...
export const HOT_REGISTRY_KEY = "__UIGHT_HOT__";
...
export interface RuntimeModuleMaps {
	 ...
	 * offering — the host learns the new index from `uight:index`, and the
...
export interface FixtureHotRegistry {
	...
	/**
	 * The re-evaluated `virtual:uight/runtime` namespace, after a file was
	 ...
	 * index itself keeps arriving over the `uight:index` event as before, so
