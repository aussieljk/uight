...
export function createSerializer(
	...
	function tryCodec(value: object, walk: Walk): Wire | null {
		for (const codec of registry) {
			...
			try {
				...
				warn(`[uight] codec "${codec.name}" threw from test(): ${String(error)}`);
				continue;
			}
			if (!matched) continue;
			...
			try {
				...
				warn(`[uight] codec "${codec.name}" threw from serialize(): ${String(error)}`);
			...
			if (dev && typeof structuredClone === "function") {
				try {
					...
					warn(
						`[uight] codec "${codec.name}" produced a value that is not structured-cloneable; falling back to an opaque chip`,
					);
					return mintOpaque(value, opaqueLabel(value), walk);
				}
			}
	...
	function serialize(value: unknown, revision: number, opts: SerializeOptions = {}): Wire {
		...
		if (dev && (walk.truncatedDepth || walk.truncatedSize)) {
			...
			walk.warn(
				`[uight] input ${name ? `"${name}"` : "(unnamed)"} was truncated by the ${what} limit; the truncated branches show as opaque chips`,
			);
		}
		...
		switch (wire.t) {
			...
			case "codec": {
				...
				if (!codec) {
					...
					warn(`[uight] unknown codec "${wire.codec}" on the wire; value dropped`);
					return { ok: false, value: undefined };
				}
				try {
					return { ok: true, value: codec.deserialize(wire.v as never) };
				} catch (error) {
					warn(`[uight] codec "${wire.codec}" threw from deserialize(): ${String(error)}`);
					return { ok: false, value: undefined };
				}
			}
			case "opaque": {
