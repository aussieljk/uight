...
export function normalizeModule(
	...
	if (isCsfFile(file, config)) {
		if (!config.storybook) {
			warn(dev, `[uight] ${file.path} looks like a CSF module but Storybook support is off`);
			return { fixtures: [] };
		}
	...
	if (defaultExport === undefined || defaultExport === null) {
		warn(dev, `[uight] ${file.path} has no default export, so it contributes no fixtures`);
		...
		for (const [name, value] of Object.entries(defaultExport)) {
			...
			if (!render) {
				warn(
					dev,
					`[uight] ${file.path}: fixture "${name}" is not an element or a component and was skipped`,
				);
				continue;
			}
		...
		warn(
			dev,
			`[uight] ${file.path}: the default export is neither an element, a component, nor an object of them`,
		);
	}
...
function reconcile(
	...
	warn(
		true,
		`[uight] ${file.path}: fixture names changed since the index was built.\n` +
