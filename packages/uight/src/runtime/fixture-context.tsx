...
function useFixtureRuntime(hook: string): FixtureRuntime {
	...
	if (!runtime) {
		throw new Error(
			`[uight] ${hook}() was called outside a fixture. Fixture hooks only work inside a fixture rendered by <Uight /> or the uight renderer.`,
		);
	}
	return runtime;
}
	...
	return React.useCallback(
		(id: FixtureId | string) => {
			...
			if (!parsed) {
				...
				console.error(`[uight] useSelectFixture(): not a fixture id: ${String(id)}`);
				return;
			}
