...
const MISSING = Symbol("uight.missing");
...
export class OverlayStore {
	...
	commitRegistration(input: {
		...
		if (this.dev && previous && previous.slot !== input.slot && previous.active) {
			...
			console.error(
				`[uight] duplicate fixture input "${input.name}"${
		...
		if (revisionChanged && previous) {
			...
			if (this.dev && bumps === 25) {
				// eslint-disable-next-line no-console
				console.warn(
					`[uight] input "${input.name}" has produced a different default on ${bumps} consecutive renders. ` +
	...
	reportDropped(name: string, revision: number, paths: readonly PathSegment[][]): void {
		...
		if (this.dev) {
			...
			// eslint-disable-next-line no-console
			console.warn(
				`[uight] ${count} overlay patch${count === 1 ? "" : "es"} for input "${name}" no longer apply to the current shape: ` +
