/**
 * Reading the options a project passes inline to `uight()`. §4.1, §4.5.
 *
 * `resolveUightConfig` answers "what did the user write in
 * `uight.config.json`", which is the wrong question for most projects: the
 * documented way to configure the plugin is an argument to the `uight()` call
 * in `vite.config.ts`, and that argument is not a file anyone can read — it is
 * an expression inside a module. `uight doctor` therefore used to report the
 ...
 *
 * **How the options get out of the call.** The plugin object `uight()` returns
 ...
 * `config()` hook, which means running uight's own scan (and every other
...
export interface ViteConfigOptions {
	...
	/** Options passed inline to `uight()`, or null when none were found. */
	options: UightPluginOptions | null;
...
/**
 * Load the project's Vite config and pull out what `uight()` was given.
 *
 ...
 */
export async function loadViteUightOptions(
	root: string,
	...
	const plugin = plugins.find(
		(entry): entry is { name: string; api?: { options?: UightPluginOptions } } =>
			typeof entry === "object" &&
			entry !== null &&
			(entry as { name?: unknown }).name === "uight",
	);
	...
	if (!plugin) {
		return {
			...
			problem: "the vite config loaded, but no uight() plugin is in its plugins array",
		};
	}
	...
	return {
		...
		problem:
			options === undefined
				? "uight() is present but exposes no options — the installed uight predates plugin.api.options"
				: null,
	};
}
