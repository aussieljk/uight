/**
 * `uight doctor` — the answer to "why is my component missing".
 *
 * §19.6 already answers that question, on `/@uight/config.json`, to a client
...
import { loadViteUightOptions } from "./load-config.ts";
 ...
 * options passed inline to `uight()` are folded into `cfg` before scanning.
...
export function formatDoctorReport(report: DoctorReport): string {
	...
	// The Vite config is loaded and its inline `uight()` options are folded in
	...
	if (report.viteConfigProblem) {
		...
		lines.push(row("", "Options below are defaults plus uight.config.json only."));
	}
	lines.push("");
	...
	if (report.problems.length === 0) {
		...
		for (const problem of report.problems) {
			lines.push(`  ${problem.kind}: ${problem.message.replace(/^\[uight\] /, "")}`);
