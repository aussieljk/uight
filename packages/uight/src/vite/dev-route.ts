...
/**
 * The minimal document. It mounts `<Uight />` and nothing else: the dev route
...
/* ------------------------------------------------------------------ *
 * `GET /@uight/preview` — the frame's document (§6.6)
 * ------------------------------------------------------------------ */
 ...
 * `#uight-root`, then injects the stylesheet and the renderer itself, so this
 * document carries no uight script of its own.
 */
...
/* ------------------------------------------------------------------ *
 * `GET /@uight/renderer` and `/@uight/dev-entry` (§6.3)
 * ------------------------------------------------------------------ */
		...
		void server
			...
			.then((result) => {
				if (!result) {
					...
					send(res, req.method, `[uight] ${id} did not resolve`);
					return;
				}
				res.statusCode = 200;
