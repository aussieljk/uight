 ...
 * uight()]` the resolved order is `vite:react-babel … @mdx-js/rollup … uight`,
...
export function checkMdxSupport(
	...
	return {
		kind: "missing",
		message:
			`[uight] found ${count} ${what} ` +
			`(e.g. ${mdxFiles[0]?.globPath ?? ""}) but no MDX plugin is installed, so Vite ` +
			`cannot compile them. §14: MDX is bundler configuration, not a uight feature — ` +
			`so uight indexes the file and the host compiles it. Add one:\n` +
