...
export function fixturePatterns(cfg: ResolvedUightConfig): string[] {
	return [`**/*.${cfg.fixtureFileSuffix}.${exts(FIXTURE_EXTENSIONS)}`];
}

/** `**\/{cosmos,uight}.decorator.{js,jsx,ts,tsx}`. */
export function decoratorPatterns(cfg: ResolvedUightConfig): string[] {
 ...
 */
export function docsPatterns(cfg: ResolvedUightConfig): string[] {
	if (!cfg.docs) return [];
	return [`**/*.${cfg.docs.fileSuffix}.mdx`];
}

/** `**\/*.stories.{js,jsx,ts,tsx}` — only when Storybook support is on (§13). */
export function storybookPatterns(cfg: ResolvedUightConfig): string[] {
	if (!cfg.storybook) return [];
	return [`**/*.${cfg.storybook.fileSuffix}.${exts(CODE_EXTENSIONS)}`];
}

export function inventoryPatterns(cfg: ResolvedUightConfig): string[] {
 ...
 */
export function inventoryIgnore(cfg: ResolvedUightConfig): string[] {
...
function insideFixturesDir(globPath: string, cfg: ResolvedUightConfig): string | null {
 ...
 */
export function fixtureGlobPatterns(cfg: ResolvedUightConfig): string[] {
...
export function decoratorGlobPatterns(cfg: ResolvedUightConfig): string[] {
	return toRootRelative(cfg, decoratorPatterns(cfg), cfg.exclude);
}

export function inventoryGlobPatterns(cfg: ResolvedUightConfig): string[] {
...
export function isFixtureFile(file: string, cfg: ResolvedUightConfig): boolean {
...
export function isDecoratorFile(file: string, cfg: ResolvedUightConfig): boolean {
	const rel = relativeToFixturesDir(file, cfg);
	if (rel === null) return false;
	return matchesAny(rel, decoratorPatterns(cfg), cfg) && passesFilters(rel, cfg);
}

export function isInventoryFile(file: string, cfg: ResolvedUightConfig): boolean {
...
export function isDocsFile(file: string, cfg: ResolvedUightConfig): boolean {
	const rel = relativeToFixturesDir(file, cfg);
	if (rel === null) return false;
	return matchesAny(rel, docsPatterns(cfg), cfg);
}

export function isCsfFile(file: string, cfg: ResolvedUightConfig): boolean {
	const rel = relativeToFixturesDir(file, cfg);
	if (rel === null) return false;
	return matchesAny(rel, storybookPatterns(cfg), cfg);
}

function relativeToFixturesDir(file: string, cfg: ResolvedUightConfig): string | null {
	const rel = path.relative(cfg.fixturesDirFsPath, path.resolve(file));
	if (rel === "" || rel.startsWith("..") || path.isAbsolute(rel)) return null;
	return rel.split(path.sep).join("/");
}

function matchesAny(rel: string, patterns: string[], cfg: ResolvedUightConfig): boolean {
...
function passesFilters(rel: string, cfg: ResolvedUightConfig): boolean {
...
export async function scanFixtures(cfg: ResolvedUightConfig): Promise<FixtureIndex> {
	...
	if (escapesRoot(cfg.fixturesDirGlobPath)) {
		problems.push({
			...
			kind: "confinement",
			message:
				`[uight] fixturesDir "${cfg.fixturesDirFsPath}" is outside the Vite root ` +
...
async function indexFixtureFile(
	...
	try {
		...
		problems?.push({
			kind: "unreadable",
			message: `[uight] could not read ${file}: ${(err as Error).message}`,
			files: [file],
		});
		return null;
	}
	...
	if (parsed.errors.length > 0) {
		problems?.push({
			kind: "unparseable",
			message: `[uight] could not parse ${file}: ${parsed.errors[0]}`,
			files: [file],
		});
	}
 ...
 */
function suffixFor(cfg: ResolvedUightConfig, csf: boolean, docsPage: boolean): string {
 ...
 */
function applyAliases(specifier: string, cfg: ResolvedUightConfig): string | null {
...
function indexDecorator(file: string, cfg: ResolvedUightConfig): DecoratorFileIndex {
	const globPath = toGlobPath(cfg.root, file);
	const rel = relativeToFixturesDir(file, cfg) ?? "";
...
export function detectCollisions(files: FixtureFileIndex[]): IndexProblem[] {
	...
	for (const [display, globPaths] of byPath) {
		...
		problems.push({
			kind: "collision",
			message:
				`[uight] ${globPaths.length} files normalize to the display path ` +
...
/* ------------------------------------------------------------------ *
 * Wire shape for the `uight:index` custom event — §4.5
 * ------------------------------------------------------------------ */
