 ...
 * decorators, the file's `uight.decorator`, and a `.storybook/preview` when one
 * is in play.
 *
 * ```ts
 * import { fixtureIds, mountFixture } from "@aussieljk/uight/test";
 *
 ...
 * `uight()` in the config. There is no second index and no second parser —
...
export interface FixtureIdsOptions {
	/** Same semantics as `<Uight filter>` (§3.6). */
	filter?: Filter;
	/**
...
async function loadPreviewEntry(): Promise<PreviewComponent> {
	...
	previewCache ??= import("virtual:uight/preview-entry").then((mod) => mod.Preview);
	return previewCache;
}

async function loadStorybookPreview(): Promise<StorybookPreview | null> {
	if (!config.hasStorybookPreview) return null;
	storybookPreviewCache ??= import("virtual:uight/storybook-preview").then(
...
export async function loadFixture(
	...
	if (!parsed) throw new Error(`[uight/test] "${String(id)}" is not a fixture id`);
	if (parsed.name === ALL_FIXTURES) {
		throw new Error(
			`[uight/test] ${parsed.path} names every fixture in the file. Pass one id, ` +
				`or map over fixtureIds({ filter: "${parsed.path}" }).`,
		);
	}

	const file = config.files.find((candidate) => candidate.path === parsed.path);
	if (!file) throw new Error(`[uight/test] no fixture file indexed at "${parsed.path}"`);

	const load = fixtureModules[file.globPath];
	if (!load) throw new Error(`[uight/test] no module registered for ${file.globPath}`);
	...
	if (!picked.fixture) {
		...
		throw new Error(
			`[uight/test] ${parsed.path} has no fixture named ${JSON.stringify(parsed.name)}. ` +
...
export async function loadComponent(item: {
	...
	if (!load) throw new Error(`[uight/test] no module registered for ${item.globPath}`);
	...
	if (
		...
		throw new Error(
			`[uight/test] ${item.globPath} has no component export named "${item.exportName}"`,
		);
	}
