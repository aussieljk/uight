...
if (argv.includes("--help") || argv.includes("-h")) {
	console.log(`uight-mcp ${version}

  --url <url>   uight dev server to read. Omit it and one is discovered by
                probing common Vite ports for /@uight/health.
...
runMcpServer({
	...
	console.error(`uight-mcp: ${error?.message ?? error}`);
	process.exit(1);
});
