...
It renders **[ljkui](https://ljkui.com)'s component library as Storybook
stories**, as fixtures, without Storybook installed, plus a handful of
hand-written fixtures covering the things Component Story Format cannot
express.

## Attribution

**ljkui is a fork of frosted-ui, Whop's design system. frosted-ui is not part
of uight, and uight is not affiliated with, endorsed by, or connected to Whop
in any way.**

frosted-ui is MIT licensed, and this demo uses it in two ways:

- It installs the published `ljkui` package as a normal dependency.
- It vendors frosted-ui's `*.stories.tsx` files into `src/stories/`, because a
  component explorer is only interesting when it is pointed at a real design
...
what was changed. Beyond the original import rewriting, the move to ljkui
renamed what the fork renamed — `Text`/`Heading`/`Code` moved under
`Typography`, `Callout` became `Alert`, `TextField` became `Input`,
`SegmentedControl` became `ToggleGroup`, `EmptyState` became `Empty`,
`DataList` became `DataTable`, `RangeCalendar` became `Calendar.Range`,
`ScrollGallery` became `Carousel`, `StackedHorizontalBarChart` became `Chart`,
and frosted-ui's sized icon exports (`Bell16`) became ljkui's icon registry
(`Icons.Bell`, drawn by a registered adapter). Two galleries had nothing left
to port and were replaced by one: `stories/storybook/ljkui-icons.stories.tsx`.
The `country-flag` story was dropped — ljkui has no such component.
