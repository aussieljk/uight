		...
		return (
			...
			>
				<Typography.Text render={<div />} style={{ maxWidth: 540, textAlign: "center" }}>
					...
					<Typography.Code>&lt;DropdownMenu.Root&gt;</Typography.Code>. When the trigger and
					menu content need to live in different parts of the tree, create a handle with{" "}
					<Typography.Code>DropdownMenu.createHandle()</Typography.Code> and pass it to both
				...
				>
					Note: Only top-level menus can have detached triggers. Submenus must have their
					triggers defined within the SubmenuRoot part.
				</Typography.Text>
		...
		return (
			...
			>
				<Typography.Text render={<div />} style={{ maxWidth: 540, textAlign: "center" }}>
					...
					<Typography.Code>&lt;DropdownMenu.Trigger&gt;</Typography.Code> components inside the
					same <Typography.Code>&lt;DropdownMenu.Root&gt;</Typography.Code>, or attach several
					detached triggers to the same handle.
				</Typography.Text>
		...
		return (
			...
			>
				<Typography.Text render={<div />} style={{ maxWidth: 600, textAlign: "center" }}>
					Control a menu's open state externally with the{" "}
					<Typography.Code>open</Typography.Code> and{" "}
					<Typography.Code>onOpenChange</Typography.Code> props. When more than one trigger can
					open the menu, track the active trigger with{" "}
					<Typography.Code>triggerId</Typography.Code> on{" "}
	...
	render: (args) => (
		...
		>
			<Typography.Text render={<div />} style={{ maxWidth: 500, textAlign: "center" }}>
				Control where the menu appears relative to the trigger using{" "}
...
export const SideOffsetAndAlignOffset: Story = {
	...
	render: (args) => (
		...
		>
			<Typography.Text render={<div />} style={{ maxWidth: 500, textAlign: "center" }}>
				Fine-tune menu positioning with <Typography.Code>sideOffset</Typography.Code>{" "}
				(distance from trigger) and <Typography.Code>alignOffset</Typography.Code> (shift
...
export const ManyAdjacentSubmenus: Story = {
	...
	render: (args) => {
		...
		return (
			...
			>
				<Typography.Text render={<div />} style={{ maxWidth: 500, textAlign: "center" }}>
					Stress test with many adjacent submenus. The menu contains 50 submenus, each with 12
					nested submenus, each containing 8 items.
				</Typography.Text>
