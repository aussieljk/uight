	...
	render: (args) => (
		...
		>
			<Typography.Text size="2" color="gray">
				Use <code>openOnHover</code> to open the popover on hover. Use <code>delay</code> to
				specify how long to wait before opening.
			</Typography.Text>
...
export const DetachedTriggers: Story = {
	...
	render: (args) => (
		...
		>
			<Typography.Text size="2" color="gray">
				Use <code>Popover.createHandle()</code> to control a popover from a trigger located
				outside the <code>Popover.Root</code>.
			</Typography.Text>
...
export const MultipleTriggersWithPayload: Story = {
	...
	render: function Render(args) {
		...
		return (
			...
			>
				<Typography.Text size="2" color="gray">
					Multiple triggers can control the same popover using a shared handle. Each trigger
					passes a typed <code>payload</code> that is inferred from the handle.
				</Typography.Text>
						...
						<Popover.Content {...args} style={{ width: 300 }}>
							...
							>
								<Avatar
									...
									color={
										payload?.role === "admin" ? "red" : payload?.role === "editor" ? "blue" : "gray"
									}
								/>
								<div>
...
export const ControlledWithMultipleTriggers: Story = {
	...
	render: function Render(args) {
		...
		const handleOpenChange = (
			isOpen: boolean,
			eventDetails: { trigger?: { id?: string } },
		) => {
			setOpen(isOpen);
		...
		return (
			...
			>
				<Typography.Text size="2" color="gray">
					Use <Typography.Code>open</Typography.Code>,{" "}
					<Typography.Code>onOpenChange</Typography.Code>, and{" "}
					<Typography.Code>triggerId</Typography.Code> for fully controlled mode with multiple
					detached triggers.
				</Typography.Text>
	...
	render: function Render(args) {
		...
		return (
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
				<Typography.Text size="2" color="gray">
					Use the <Typography.Code>anchor</Typography.Code> prop to position the popover
					relative to a different element than the trigger. This is useful for tooltips on
					specific parts of a component, or when you want visual separation between the trigger
					and the anchored content.
				</Typography.Text>
...
export const DisableAnchorTracking: Story = {
	...
	render: function Render(args) {
		...
		return (
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
				<Typography.Text size="2" color="gray">
					The <Typography.Code>disableAnchorTracking</Typography.Code> prop controls whether
					the popover repositions when the anchor element moves due to layout changes (not
					scrolling). Open both popovers and click the button to move the anchors.
				</Typography.Text>
...
export const Sticky: Story = {
	...
	render: function Render(args) {
		return (
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
				<Typography.Text size="2" color="gray">
					The <Typography.Code>sticky</Typography.Code> prop controls whether the popover stays
					visible in the viewport when the anchor is scrolled out of view. Open the popover
					then scroll this page to see the effect.
				</Typography.Text>
...
export const CollisionBoundary: Story = {
	...
	render: function Render(args) {
		...
		return (
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
				<Typography.Text size="2" color="gray">
					The <Typography.Code>collisionBoundary</Typography.Code> prop defines the area within
					which the popover should stay. By default, it&apos;s the viewport. You can set it to
					a specific element to constrain the popover within that container.
				</Typography.Text>
...
export const CollisionAvoidanceDemo: Story = {
	...
	render: function Render(args) {
		...
		return (
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
				<Typography.Text size="2" color="gray">
					The <Typography.Code>collisionAvoidance</Typography.Code> prop controls how the
					popover avoids collisions with the boundary. You can configure behavior for both the{" "}
					<Typography.Code>side</Typography.Code> axis (flip or shift) and the{" "}
					<Typography.Code>align</Typography.Code> axis.
				</Typography.Text>
				...
				<Typography.Text size="1" color="gray">
					The dashed box is set as the collision boundary. Try positioning your browser so the
					triggers are near the edge of the boundary.
				</Typography.Text>
			</div>
		);
	},
...
export const Modal: Story = {
	...
	render: function Render(args) {
		return (
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
				<Typography.Text size="2" color="gray">
					The <Typography.Code>modal</Typography.Code> prop on{" "}
					<Typography.Code>Popover.Root</Typography.Code> controls how the popover interacts
					with the rest of the page. Open each popover and try interacting with the input
					below.
				</Typography.Text>
				...
				<div style={{ display: "flex", gap: "var(--space-6)", flexWrap: "wrap" }}>
						...
						<Popover.Root modal={false}>
							...
							<Popover.Content {...args}>
								<Typography.Text size="2">
									Click outside or interact with other elements freely.
								</Typography.Text>
							</Popover.Content>
						</Popover.Root>
					</div>
					...
					<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
						...
						<Popover.Root modal={true}>
							...
							<Popover.Content {...args}>
								<Typography.Text size="2">
									Page scroll is locked. Click outside to close.
								</Typography.Text>
							</Popover.Content>
						</Popover.Root>
					</div>
					...
					<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
						...
						<Popover.Root modal="trap-focus">
							...
							<Popover.Content {...args}>
								<Typography.Text size="2">
									Focus is trapped but you can scroll the page.
								</Typography.Text>
							</Popover.Content>
						</Popover.Root>
					</div>
		...
		return (
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
				<Typography.Text size="2" color="gray">
					...
					complete). Open and close the popover to see the timing difference.
				</Typography.Text>

				<Popover.Root
					onOpenChange={(open) => addLog(`onOpenChange: ${open ? "opening" : "closing"}`)}
					onOpenChangeComplete={(open) =>
						addLog(`onOpenChangeComplete: ${open ? "opened" : "closed"}`)
					}
				>
					<Popover.Trigger>
					...
					{logs.length === 0 ? (
						...
						logs.map((log, i) => (
							<div
								key={i}
								style={{
									color: log.includes("Complete") ? "var(--accent-11)" : "var(--gray-11)",
								}}
							>
								{log}
							</div>
				...
				<Typography.Text size="1" color="gray">
					...
					<br />
					<Typography.Code>onOpenChangeComplete</Typography.Code> fires after the enter/exit
					animation finishes.
				</Typography.Text>
			</div>
		);
...
export const ActionsRef: Story = {
	...
	render: function Render(args) {
		...
		return (
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
				<Typography.Text size="2" color="gray">
					The <Typography.Code>actionsRef</Typography.Code> prop provides imperative control
					over the popover. Use <Typography.Code>close()</Typography.Code> to programmatically
					close the popover after a form submission or async action completes.
				</Typography.Text>
				...
				<Typography.Text size="1" color="gray">
					The popover closes automatically after successful form submission or when an action
					completes.
				</Typography.Text>
			</div>
		);
...
// Changed from upstream: `Record<string, unknown>` does not accept an interface
// without an index signature, so this now names the props it is actually given.
function QuickActionsDemo({
	args,
}: {
	args: React.ComponentProps<typeof Popover.Content>;
}) {
...
export const FinalFocus: Story = {
	...
	render: function Render(args) {
		...
		return (
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
				<Typography.Text size="2" color="gray">
					The <Typography.Code>finalFocus</Typography.Code> prop controls where focus returns
					after the popover closes. By default, focus returns to the trigger element.
				</Typography.Text>
				...
				<div style={{ display: "flex", gap: "var(--space-6)", flexWrap: "wrap" }}>
					<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
						...
						<Popover.Root>
							...
							<Popover.Content {...args}>
								<Typography.Text size="2">
									Close me and focus returns to the trigger button.
								</Typography.Text>
								<Popover.Close>
									<Button size="1" variant="soft" style={{ marginTop: 12 }}>
										Close
					...
					<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
						...
						<Popover.Root>
							...
							<Popover.Content {...args} finalFocus={false}>
								<Typography.Text size="2">
									Close me and focus stays where it is (not moved).
								</Typography.Text>
								<Popover.Close>
									<Button size="1" variant="soft" style={{ marginTop: 12 }}>
										Close
						...
						<div style={{ display: "flex", gap: "var(--space-2)", alignItems: "center" }}>
							<Popover.Root>
								...
								<Popover.Content {...args} finalFocus={alternateRef}>
									<Typography.Text size="2">
										Close me and focus moves to the &quot;Target&quot; button.
									</Typography.Text>
									<Popover.Close>
										<Button size="1" variant="soft" style={{ marginTop: 12 }}>
											Close
				...
				>
					...
					<Input.Control
						ref={inputRef}
						placeholder="Focus lands here when pressing Escape..."
					/>
				</div>

				<Typography.Text size="1" color="gray">
					The <Typography.Code>finalFocus</Typography.Code> function receives the interaction
					type (<Typography.Code>keyboard</Typography.Code>,{" "}
					<Typography.Code>pointer</Typography.Code>, <Typography.Code>touch</Typography.Code>)
					so you can customize behavior based on how the popover was closed.
				</Typography.Text>
			</div>
		);
	},
...
export const InitialFocus: Story = {
	...
	render: function Render(args) {
		...
		return (
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
				<Typography.Text size="2" color="gray">
					The <Typography.Code>initialFocus</Typography.Code> prop controls which element
					receives focus when the popover opens. By default, focus moves to the first focusable
					...
					<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
						...
						<Popover.Root>
							...
							<Popover.Content {...args} initialFocus={false} style={{ width: 260 }}>
								...
								<Typography.Text size="2" color="gray">
									This is a read-only info panel. No element receives focus when it opens, which
									can be useful for non-interactive popovers.
								</Typography.Text>
								<Popover.Close>
									<Button size="1" variant="soft" style={{ marginTop: 12 }}>
										Got it
				...
				<Typography.Text size="1" color="gray">
					The <Typography.Code>initialFocus</Typography.Code> function receives the interaction
					type (<Typography.Code>keyboard</Typography.Code>,{" "}
					<Typography.Code>pointer</Typography.Code>, <Typography.Code>touch</Typography.Code>)
					so you can customize initial focus based on how the popover was opened.
				</Typography.Text>
			</div>
		);
	},
