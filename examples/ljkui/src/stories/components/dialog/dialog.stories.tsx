...
export const DetachedTriggers: Story = {
	...
	render: (args) => (
		...
		>
			<Typography.Text>
				Use <Typography.Code>Dialog.createHandle()</Typography.Code> to control a dialog from
				a trigger located outside the <Typography.Code>Dialog.Root</Typography.Code>.
			</Typography.Text>
...
export const MultipleTriggers: Story = {
	...
	render: function Render(args) {
		const handle = React.useMemo(
			() => Dialog.createHandle<{ title: string; mode: "create" | "edit" | "view" }>(),
			[],
		);
		...
		return (
			...
			>
				<Typography.Text>
					...
					handle - TypeScript will catch type errors.
				</Typography.Text>
...
export const ActionsRef: Story = {
	...
	render: function Render(args) {
		...
		return (
			...
			>
				<Typography.Text>
					Use <Typography.Code>actionsRef</Typography.Code> to close the dialog
			...
			>
				...
				<Dialog.Root>
					...
					<Dialog.Content {...args} style={{ maxWidth: 400 }} initialFocus={nameInputRef}>
						...
						<Dialog.Description>
							Focus is automatically set to the Name input when the dialog opens.
						</Dialog.Description>
				...
				<div style={{ display: "flex", gap: "var(--space-2)", alignItems: "center" }}>
					<Dialog.Root>
						...
						<Dialog.Content {...args} style={{ maxWidth: 400 }} finalFocus={buttonRef}>
							...
							<Dialog.Description>
								When this dialog closes, focus will return to the "Focus Here" button instead of
								the trigger.
							</Dialog.Description>
							<div
								style={{
		...
		return (
			...
			>
				<Typography.Text style={{ maxWidth: 500, textAlign: "center" }}>
					Dialogs can be nested within one another. The parent dialog dims when a child dialog
					opens.
				</Typography.Text>
...
export const CloseConfirmation: Story = {
	...
	render: function Render(args) {
		...
		return (
			...
			>
				<Typography.Text style={{ maxWidth: 500, textAlign: "center" }}>
					This demo shows how to use a nested <Typography.Code>AlertDialog</Typography.Code>{" "}
					inside a <Typography.Code>Dialog</Typography.Code> to confirm discarding unsaved
					changes. Type something in the textarea, then try to close the dialog.
				</Typography.Text>
...
export const WithPopoverAndHoverCard: Story = {
	...
	render: function Render(args) {
		return (
			...
			>
				<Typography.Text style={{ maxWidth: 500, textAlign: "center" }}>
					Dialogs can contain other floating elements like Popovers and HoverCards. Focus
...
export const DialogFromDropdownMenu: Story = {
	...
	render: function Render(args) {
		...
		return (
			...
			>
				<Typography.Text style={{ maxWidth: 500, textAlign: "center" }}>
					Open a dialog from a dropdown menu using controlled state. The dialog is controlled
					via <Typography.Code>open</Typography.Code> and{" "}
					<Typography.Code>onOpenChange</Typography.Code> props, and opened imperatively via{" "}
					<Typography.Code>onClick</Typography.Code> on the menu item.
				</Typography.Text>
					...
					<Dialog.Content {...args} style={{ maxWidth: 450 }}>
						...
						>
							...
							<label>
								...
								<Textarea
									defaultValue="A sample project description"
				...
				<Typography.Text render={<div />} style={{ maxWidth: 540, textAlign: "center" }}>
					...
					<ul style={{ textAlign: "left", marginTop: "var(--space-2)" }}>
						<li>
							...
							<Typography.Code>DropdownMenu.Content</Typography.Code> so the Dialog.Root stays
							mounted when the menu closes.
						</li>
						<li>
							Add <Typography.Code>tabIndex=&#123;-1&#125;</Typography.Code> to the trigger to
							prevent it from stealing focus when the dropdown opens (preserves roving focus).
						</li>
					</ul>
				</Typography.Text>
	...
	render: function Render(args) {
		...
		return (
			...
			>
				...
				>
					Open dialogs directly from dropdown menu items using detached triggers with handles.
					Create a handle with <Typography.Code>Dialog.createHandle()</Typography.Code>, wrap
					the menu item with <Typography.Code>Dialog.Trigger</Typography.Code> passing the
					handle, then connect it to a <Typography.Code>Dialog.Root</Typography.Code> with the
					same handle. Add <Typography.Code>tabIndex=&#123;-1&#125;</Typography.Code> to the
					trigger to preserve keyboard navigation in the menu.
				</Typography.Text>
