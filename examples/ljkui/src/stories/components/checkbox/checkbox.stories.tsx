	...
	render: (args) => (
		<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
			<Typography.Text style={{ marginBottom: 12 }}>
				Composing <Typography.Code>Checkbox</Typography.Code> within{" "}
			...
			<div style={{ maxWidth: 300 }}>
				<Typography.Text render={<label />} size="4">
					<div style={{ display: "flex", gap: "var(--space-2)" }}>
						<Checkbox defaultChecked /> It is automatically well-aligned with multi-line text
						too.
					</div>
				</Typography.Text>
			</div>
		...
		return (
			...
			>
				<Typography.Text>
					...
					<Typography.Code>uncheckedValue</Typography.Code> props for form submissions. By
					default, a checked checkbox submits <Typography.Code>"on"</Typography.Code> and an
					unchecked checkbox submits nothing (standard HTML behavior). These props let you
					customize both values.
				</Typography.Text>
				<form
				...
				<div>
					...
					<Typography.Code
						style={{ display: "block", marginTop: "var(--space-2)", whiteSpace: "pre" }}
					>
...
export const ReadOnly: Story = {
	...
	render: (args) => {
		...
		return (
			...
			>
				<Typography.Text>
					The <Typography.Code>readOnly</Typography.Code> prop prevents users from changing a
					checkbox while still showing its current state. Unlike{" "}
					<Typography.Code>disabled</Typography.Code>, read-only checkboxes remain focusable
					and their values are submitted with forms. This is useful for displaying user
					preferences they cannot modify, or showing computed/derived states.
				</Typography.Text>
				<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
					<Typography.Text size="2" weight="medium">
				...
				<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
					...
					<Typography.Text render={<label />} size="2">
						<div style={{ display: "flex", gap: "var(--space-2)", alignItems: "center" }}>
							<Checkbox {...args} checked={preferences.emailDigest} readOnly /> Daily email
							digest
						</div>
					</Typography.Text>
					<Typography.Text render={<label />} size="2">
					...
					<Typography.Text render={<label />} size="2">
						<div style={{ display: "flex", gap: "var(--space-2)", alignItems: "center" }}>
							<Checkbox {...args} checked={preferences.pushNotifications} readOnly /> Push
							notifications
						</div>
					</Typography.Text>
				</div>
		...
		return (
			...
			>
				...
				<form>
					<Typography.Text render={<label />} size="2">
						<div style={{ display: "flex", gap: "var(--space-2)", alignItems: "center" }}>
							<Checkbox {...args} inputRef={inputRef} name="agreement" required /> I agree to the
							terms (required)
						</div>
					</Typography.Text>
				</form>
