	...
	render: (args) => (
		<div style={{ display: "flex", gap: "var(--space-6)", alignItems: "flex-start" }}>
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
				<span style={{ fontSize: "var(--font-size-2)", color: "var(--gray-11)" }}>
					Horizontal
				</span>
				<div style={{ width: 200 }}>
...
export const ValueCallbacks: Story = {
	...
	render: (args) => {
		...
		return (
			...
			>
				<Typography.Text>
					<Typography.Code>onValueChange</Typography.Code> fires continuously while dragging —
					ideal for live previews.
					<br />
					<Typography.Code>onValueCommitted</Typography.Code> fires only when dragging ends —
					ideal for saving or API calls.
				</Typography.Text>
...
export const MinStepsBetweenValues: Story = {
	...
	render: (args) => {
		...
		return (
			...
			>
				<Typography.Text>
					The <Typography.Code>minStepsBetweenValues</Typography.Code> prop ensures a minimum
					gap between thumbs in a range slider. Combined with{" "}
					<Typography.Code>thumbCollisionBehavior="push"</Typography.Code>, when one thumb
					approaches another, it pushes it along rather than stopping — creating a smooth,
					intuitive interaction while maintaining the minimum gap.
				</Typography.Text>
				...
				<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
					...
					<Typography.Text size="2" color="gray">
						Available: {formatTime(timeRange[0])} – {formatTime(timeRange[1])} (
						{timeRange[1] - timeRange[0]} hours)
					</Typography.Text>
				</div>
				...
				<Typography.Text size="1" color="gray">
					Other collision behaviors: <Typography.Code>"swap"</Typography.Code> (thumbs swap
					places) and <Typography.Code>"none"</Typography.Code> (thumbs block each other).
				</Typography.Text>
			</div>
		);
	},
		...
		return (
			...
			>
				<Typography.Text>
					...
					hidden native <Typography.Code>&lt;input type="range"&gt;</Typography.Code> element
					inside the thumb. This is useful for programmatic focus, form validation, or
				...
				<Typography.Code style={{ padding: "var(--space-2)", display: "block" }}>
					{info}
				</Typography.Code>
			</div>
		);
