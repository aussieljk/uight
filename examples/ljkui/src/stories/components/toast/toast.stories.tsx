...
export const Info: Story = {
	render: () => (
		<Button
			onClick={() => toast.info("Camera access denied. Call will continue with voice only.")}
		>
			Show info toast
		</Button>
	),
...
export const Loading: Story = {
	render: () => (
		<Button onClick={() => toast.loading("Connecting to server...")}>
			Show loading toast
		</Button>
	),
};
...
export const Default: Story = {
	render: () => (
		<Button onClick={() => toast("A new notification has arrived")}>
			Show default toast
		</Button>
	),
};
			...
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
				<div style={{ maxWidth: 480 }}>
					<Typography.Text size="2" color="gray">
						<Typography.Code size="1">toast.promise</Typography.Code> conditionally skips toast
						creation when an option is <Typography.Code size="1">undefined</Typography.Code>. If{" "}
						<Typography.Code size="1">loading</Typography.Code> is undefined, no loading spinner
						appears. If <Typography.Code size="1">success</Typography.Code> or{" "}
						<Typography.Code size="1">error</Typography.Code> is undefined, the loading toast
						(if any) is dismissed silently. This matches sonner&apos;s behavior.
					</Typography.Text>
				</div>
				<div style={{ display: "flex", gap: "var(--space-3)", flexWrap: "wrap" }}>
					<Button
							...
							const interval = setInterval(() => {
								setProgress((prev) => {
									...
									if (next >= 100) {
										...
										timeout = setTimeout(
											() => toast.success("design-system-v2.fig uploaded", { id }),
											400,
										);
									}
									return next;
...
export const CustomDuration: Story = {
	...
	render: () => (
		<div style={{ display: "flex", gap: "var(--space-3)" }}>
			...
			<Button onClick={() => toast.success("Quick toast", { duration: 1500 })}>
				Short duration (1.5s)
			</Button>
		</div>
	),
...
export const VaryingHeights: Story = {
	...
	render: () => {
		...
		function createToast() {
			count += 1;
			const description =
				VARYING_DESCRIPTIONS[Math.floor(Math.random() * VARYING_DESCRIPTIONS.length)];
...
export const CustomPosition: Story = {
	...
	render: () => (
		...
		>
			<div style={{ display: "flex", gap: "var(--space-3)" }}>
				<Button onClick={() => toast.success("Top left", { position: "top-left" })}>
					Top Left
				</Button>
				<Button onClick={() => toast.info("Top center", { position: "top-center" })}>
					Top Center
				</Button>
				<Button onClick={() => toast.error("Top right", { position: "top-right" })}>
					Top Right
				</Button>
			</div>
			<div style={{ display: "flex", gap: "var(--space-3)" }}>
				<Button onClick={() => toast.success("Bottom left", { position: "bottom-left" })}>
					Bottom Left
				</Button>
				<Button onClick={() => toast("Bottom center", { position: "bottom-center" })}>
					Bottom Center
				</Button>
				<Button onClick={() => toast.error("Bottom right", { position: "bottom-right" })}>
					Bottom Right
				</Button>
			</div>
		</div>
	...
	render: () => {
		...
		return (
			<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
				<div style={{ maxWidth: 480 }}>
					<Typography.Text size="2" color="gray">
						Passing an explicit <Typography.Code size="1">id</Typography.Code> prevents
						duplicate toasts. If a toast with that ID already exists, it updates in place
						instead of creating a new one and plays a subtle bounce animation so the user
						notices the update. This is useful for recurring events like network errors or
						polling failures where you want one persistent toast rather than a growing stack.
					</Typography.Text>
				</div>
				<div style={{ display: "flex", gap: "var(--space-3)", flexWrap: "wrap" }}>
					<Button
...
export const CustomContent: Story = {
	...
	render: () => (
		<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
			<div style={{ maxWidth: 480 }}>
				<Typography.Text size="2" color="gray">
					<Typography.Code size="1">toast.custom()</Typography.Code> accepts a render callback
					for fully custom toast content. The callback receives{" "}
					<Typography.Code size="1">{"{ close, id, Toast }"}</Typography.Code> where{" "}
					<Typography.Code size="1">Toast</Typography.Code> provides{" "}
					<Typography.Code size="1">Root</Typography.Code>,{" "}
					<Typography.Code size="1">Content</Typography.Code>,{" "}
					<Typography.Code size="1">Title</Typography.Code>, and{" "}
					<Typography.Code size="1">Description</Typography.Code> sub-components. Compose them
					as{" "}
					<Typography.Code size="1">
						{"<Toast.Root><Toast.Content>…</Toast.Content></Toast.Root>"}
					</Typography.Code>{" "}
					to get the standard chrome (animations, swipe-to-dismiss, stacking). Use{" "}
					<Typography.Code size="1">Toast.Title</Typography.Code> and{" "}
					<Typography.Code size="1">Toast.Description</Typography.Code> for consistent
					typography. Pass <Typography.Code size="1">className</Typography.Code> or{" "}
					<Typography.Code size="1">style</Typography.Code> to any component to restyle it.
				</Typography.Text>
			</div>
			<div style={{ display: "flex", gap: "var(--space-3)", flexWrap: "wrap" }}>
				<Button
