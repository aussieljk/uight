...
import { Button, IconButton, Kbd, Tooltip, type TooltipActions, Typography } from "ljkui";
	...
	render: () => (
		...
		>
			<Typography.Text render={<div />} style={{ maxWidth: 400, textAlign: "center" }}>
				Wrap tooltips in <Typography.Code>Tooltip.Provider</Typography.Code> for shared delay
				behavior. After hovering one tooltip, subsequent tooltips in the group open instantly.
			</Typography.Text>
	...
	render: () => (
		...
		>
			<Typography.Text render={<div />} style={{ maxWidth: 400, textAlign: "center" }}>
				...
				<Typography.Code>align</Typography.Code> props to control tooltip positioning.
			</Typography.Text>
	...
	render: () => (
		...
		>
			<Typography.Text render={<div />} style={{ maxWidth: 400, textAlign: "center" }}>
				...
				<Typography.Code>disableHoverablePopup=true</Typography.Code> to disable this
				behavior.
			</Typography.Text>
			...
			<div style={{ display: "flex", gap: "var(--space-3)" }}>
				...
				<Tooltip
					content="I close immediately when you leave the trigger"
		...
		return (
			...
			>
				<Typography.Text render={<div />} style={{ maxWidth: 460, textAlign: "center" }}>
					...
					<Typography.Code>closeOnClick={"{false}"}</Typography.Code> to keep the tooltip
					visible after the click — useful when the tooltip describes an action that does not
					navigate or remove the trigger from the layout.
				</Typography.Text>
...
export const Disabled: Story = {
	...
	render: function Render() {
		return (
			...
			>
				<Typography.Text render={<div />} style={{ maxWidth: 460, textAlign: "center" }}>
					Set <Typography.Code>disabled={"{true}"}</Typography.Code> to prevent the tooltip
					from opening regardless of trigger interactions.
				</Typography.Text>
...
export const SideOffset: Story = {
	...
	render: () => (
		...
		>
			<Typography.Text render={<div />} style={{ maxWidth: 400, textAlign: "center" }}>
				Adjust the distance from the trigger with{" "}
				<Typography.Code>sideOffset</Typography.Code>.
			</Typography.Text>
...
export const TrackCursorAxis: Story = {
	...
	render: () => (
		...
		>
			<Typography.Text render={<div />} style={{ maxWidth: 400, textAlign: "center" }}>
				Make the tooltip follow your cursor with{" "}
				<Typography.Code>trackCursorAxis</Typography.Code>. Move your mouse over the buttons
				to see the effect.
			</Typography.Text>
		...
		return (
			...
			>
				<Typography.Text render={<div />} style={{ maxWidth: 450, textAlign: "center" }}>
					<Typography.Code>onOpenChange</Typography.Code> fires immediately when the tooltip
					starts opening/closing. <Typography.Code>onOpenChangeComplete</Typography.Code> fires
					...
					{events.length === 0 ? (
						...
						events.map((entry, i) => (
							<div key={i} style={{ opacity: 0.5 + (i / events.length) * 0.5, marginBottom: 2 }}>
								<span style={{ color: "var(--gray-11)" }}>{entry.time}</span>{" "}
...
export const ActionsRef: Story = {
	...
	render: function Render() {
		...
		return (
			...
			>
				<Typography.Text render={<div />} style={{ maxWidth: 400, textAlign: "center" }}>
					Use <Typography.Code>actionsRef</Typography.Code> to imperatively control the
					tooltip. The <Typography.Code>close()</Typography.Code> method closes the tooltip
					programmatically.
				</Typography.Text>
