...
const articleImages = [
	...
	},
	{
		...
		caption:
			"Haukland Beach — often called the most beautiful beach above the Arctic Circle",
	},
	{
		id: "aurora",
...
const ArticleImage = ({
	...
	return (
		...
		>
			<Lightbox.Trigger
				index={index}
				style={{ ...triggerStyle, display: "block", width: "100%" }}
			>
				<img
					src={image.inline}
					alt={image.alt}
...
export const WithinArticle: Story = {
	...
	render: () => (
		<Lightbox.Root
			viewTransition
			scrollTriggerIntoView={{ type: "onChange", behavior: "smooth" }}
		>
			<article style={{ maxWidth: 680, margin: "0 auto", lineHeight: 1.7 }}>
				...
				<Typography.Text render={<p />} size="3" style={{ marginBottom: "var(--space-4)" }}>
					...
					a remarkably mild climate thanks to the Gulf Stream — winter temperatures rarely drop
					below -5°C, and summer brings the midnight sun for weeks on end.
				</Typography.Text>
				...
				<Typography.Text render={<p />} size="3" style={{ marginBottom: "var(--space-4)" }}>
					...
					wooden drying racks (hjell) still dominate the landscape from February to June, their
					skeletal frames hung with thousands of split cod destined to become stockfish,
				...
				<Typography.Text render={<p />} size="3" style={{ marginBottom: "var(--space-4)" }}>
					...
					just 1,161 metres — but their proximity to the sea makes them feel immense.
					Ridgelines launch almost vertically from the shoreline, and many of the most
					rewarding hikes start at sea level. Reinebringen, perhaps the most famous, climbs 448
					metres via a recently built Sherpa staircase and rewards hikers with a panorama that
					frequently appears on "best views in the world" lists.
				</Typography.Text>
				...
				<Typography.Text render={<p />} size="3" style={{ marginBottom: "var(--space-4)" }}>
					...
					for the snow-dusted mountains framing them. The water's turquoise tint is real —
					caused by the white sand bottom and the clarity of the Arctic Sea — though a swim
					will quickly remind you that this is not, in fact, the tropics.
				</Typography.Text>
				...
				<Typography.Text render={<p />} size="3" style={{ marginBottom: "var(--space-4)" }}>
					Photographers flock to Lofoten for the light. From late May to mid-July, the sun
					never sets, casting a warm golden glow that rolls around the horizon through the
					night. In the polar night of December and January, the sun never rises, but the
					twilight hours produce extraordinary pastel skies that last for hours. And then
					there's the aurora.
				</Typography.Text>
				...
				<Typography.Text render={<p />} size="3" style={{ marginBottom: "var(--space-4)" }}>
					...
					welcoming visitors while preserving the pristine landscape and traditional way of
					life that make the islands special. Sustainable tourism initiatives, strict building
					codes, and a deep cultural connection to the sea offer reasons for optimism.
				</Typography.Text>
...
const feedPosts: FeedPost[] = [
	{
		...
		text:
			"Spent the weekend exploring the old quarter. Every alley has a story — and apparently, a very photogenic cat.",
		time: "2h",
		images: [
			{
	...
	},
	{
		...
		text:
			"New studio setup is finally complete. Natural light in the morning, controlled lighting at night. Couldn't be happier with how these turned out.",
		time: "5h",
		images: [
			{
	...
	},
	{
		...
		text:
			"Food market finds from this morning. Everything was so fresh the vendors were still arranging displays when I arrived. Got there at 6am and it was worth every lost minute of sleep.",
		time: "8h",
		images: [
			{
	...
	},
	{
		...
		text:
			"Architecture walk through the brutalist district. Say what you will about the aesthetic — the geometry is unreal in the right light.",
		time: "12h",
		images: [
			{
	...
	},
	{
		...
		text:
			"First snowfall of the season, and naturally I had my camera. The way the light hits fresh powder at golden hour is something else entirely.",
		time: "1d",
		images: [
			{
	...
	},
	{
		...
		text:
			"Week-long road trip through the coast is done. Seven stops, seven completely different landscapes. This country never stops surprising me.",
		time: "2d",
		images: [
			{
		...
		>
			{imgs.slice(0, 4).map((img, i) => (
				<Lightbox.Trigger
					key={i}
					index={i}
					crossfade={i === 3 && extra > 0}
					style={feedTriggerBase}
				>
					<div
...
function ActionBar({ stats }: { stats: FeedPost["stats"] }) {
	return (
		...
		>
			{[
				{ icon: <Icons.Message />, count: stats.replies },
				{ icon: <Icons.Heart />, count: stats.likes },
				{ icon: <Icons.Upload />, count: stats.shares },
			].map((action, i) => (
...
function FeedPostCard({
	...
	return (
		<Lightbox.Root viewTransition morphTo={morphTo}>
			<article
				style={{ display: "flex", gap: "var(--space-3)", padding: "var(--space-4) 0" }}
			>
...
function ScreenCard({
	...
	screen: (typeof designScreens)[number];
	index: number;
}) {
	return (
				...
				>
					...
					>
						...
						>
							...
							<div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
								<Avatar
									size="3"
									src="https://i.pravatar.cc/150?u=agent-claire"
									fallback="CR"
									shape="circle"
								/>
								<div>
					...
					<div style={{ marginTop: "var(--space-6)" }}>
						...
						<Typography.Text
							render={<p />}
							size="3"
							style={{ lineHeight: 1.7, marginBottom: "var(--space-3)" }}
						>
							Perched on a sun-drenched hillside with unobstructed ocean and mountain views, The
							Olive Grove Residence is a masterful blend of contemporary architecture and
							Mediterranean warmth. Designed by acclaimed architect Rafael Mendes, the home flows
							seamlessly between indoor and outdoor living through walls of retractable glass
							that frame the Santa Ynez Mountains to the north and the Pacific to the south.
						</Typography.Text>
						<Typography.Text
							render={<p />}
							size="3"
			...
			<div style={{ maxWidth: 780, margin: "0 auto", padding: "var(--space-4) 0" }}>
				<div style={{ marginBottom: "var(--space-5)" }}>
					...
					<Typography.Text size="3" color="gray" style={{ display: "block" }}>
						A curated palette of colors found in nature. Click any swatch to reveal its
						photograph.
					</Typography.Text>
				</div>
...
const films = [
	{
		...
		synopsis:
			"A giant rabbit deals with three bullying rodents in this landmark open-source animated short.",
	},
	{
		id: "ed",
		...
		synopsis:
			"Two characters explore a strange mechanical world, questioning reality and their relationship.",
	},
	{
		id: "sintel",
		title: "Sintel",
		...
		synopsis:
			"A lone warrior searches for a baby dragon she befriended, facing dangerous adversaries along the way.",
	},
	{
		id: "tos",
		...
		synopsis:
			"In a dystopian future, a group of warriors and scientists attempt to reverse an apocalyptic event.",
	},
	{
		id: "subaru",
		...
		synopsis:
			"A short film showcasing the versatility of driving through diverse landscapes and terrain.",
	},
	{
		id: "fbj",
		...
		synopsis:
			"A high-octane teaser that puts you behind the wheel for an adrenaline-pumping ride.",
	},
];
			...
			<div style={{ maxWidth: 720, margin: "0 auto" }}>
				<div style={{ padding: "var(--space-5) 0 var(--space-4)" }}>
					...
					<Typography.Text
						size="2"
						color="gray"
						style={{ marginTop: 8, display: "block", maxWidth: 480 }}
					>
						A curated collection of award-winning open-source short films. Click any poster to
						watch the trailer.
					</Typography.Text>
				</div>
				...
				<Lightbox.Root viewTransition>
					...
					<Lightbox.Content>
						...
						>
							<Tooltip content="Close">
								<Lightbox.Close
									render={<IconButton variant="ghost" color="gray" size="3" aria-label="Close" />}
								>
									<Icons.Close />
								</Lightbox.Close>
							</Tooltip>
						...
						>
							<Tooltip content="Previous">
								<Lightbox.Previous
									render={
										<IconButton variant="ghost" color="gray" size="3" aria-label="Previous" />
									}
								>
									<Icons.ChevronLeft />
								</Lightbox.Previous>
						...
						>
							<Tooltip content="Next">
								<Lightbox.Next
									render={<IconButton variant="ghost" color="gray" size="3" aria-label="Next" />}
								>
									<Icons.ChevronRight />
								</Lightbox.Next>
							</Tooltip>
	...
	return (
		...
		>
			...
			>
				...
				<Typography.Text
					size="2"
					style={{ display: "block", marginTop: "var(--space-3)", lineHeight: 1.6 }}
				>
					{member.bio}
				</Typography.Text>
				...
				<div style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
					<div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)" }}>
						<Icons.MapPin style={{ color: "var(--gray-a10)" }} />
						<Typography.Text size="2" color="gray">
							{member.location}
						</Typography.Text>
					</div>
					<div style={{ display: "flex", alignItems: "center", gap: "var(--space-2)" }}>
						<Icons.Mail style={{ color: "var(--gray-a10)" }} />
						<Typography.Text size="2" color="gray">
							{member.email}
						</Typography.Text>
					</div>
			...
			<div style={{ maxWidth: 640, margin: "0 auto" }}>
				<div style={{ padding: "var(--space-5) 0 var(--space-4)" }}>
					...
					<Typography.Text
						size="2"
						color="gray"
						style={{ marginTop: 8, display: "block", maxWidth: 440 }}
					>
			...
			>
				<Lightbox.Root
					...
					onOpenChangeComplete={(open) => {
						addLog(
							open ? "onOpenChangeComplete(true)" : "onOpenChangeComplete(false)",
							"complete",
						);
					}}
				>
				...
				>
					...
					{logs.map((log) => (
						...
						>
							...
							<Typography.Text
								size="1"
								style={{ color: log.type === "change" ? "var(--blue-11)" : "var(--green-11)" }}
							>
								{log.label}
							</Typography.Text>
						</div>
...
export const WithZoom: Story = {
	name: "With Zoom",
	render: () => {
		const zoomRef = useRef<Lightbox.LightboxZoomRef>(null);
									...
									overlay={
										...
										>
											<Tooltip content="Zoom out">
												<Lightbox.ZoomOut
													render={<IconButton variant="ghost" color="gray" size="2" />}
												>
			...
			>
				{morphDemoImages.slice(0, triggerCount).map((img, i) => (
					<Lightbox.Trigger
						...
						index={i}
						crossfade={
							crossfadeTriggers === "all" ||
							(crossfadeTriggers === "last" && i === triggerCount - 1)
						}
	...
	render: () => (
		...
		>
			<div>
				...
				>
					Default behavior. On close the lightbox morphs back to the trigger that matches the
					current active index. All five images have triggers, so every item has a target.
					Navigate to any image and close — the morph always lands on the matching trigger.
				</Typography.Text>
			...
			<div>
				...
				<Typography.Text
					...
					style={{ marginBottom: "var(--space-4)", lineHeight: 1.6 }}
				>
					Always morph back to the trigger that originally opened the lightbox, regardless of
					which item is active when closing. Useful when there is only one trigger (e.g. a
					message attachment stack) — no matter how far you navigate, the close animation
					always returns to the opening trigger.
				</Typography.Text>
			...
			<div>
				...
				<Typography.Text
					...
					style={{ marginBottom: "var(--space-4)", lineHeight: 1.6 }}
				>
					Morph to the trigger at the active index if it exists; otherwise fall back to the
					nearest registered trigger. Below, only the first 3 of 5 images have visible
					triggers. Navigate to image 4 or 5 and close — the morph falls back to trigger 3 (the
					closest). Great for grids with a &quot;+N more&quot; overlay.
				</Typography.Text>
...
// ---------------------------------------------------------------------------
// Typography.Text Message with Image Stack
// ---------------------------------------------------------------------------
