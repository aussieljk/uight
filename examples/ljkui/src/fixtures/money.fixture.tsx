...
import { Badge, Card, DataTable, Typography } from "ljkui";
...
export default function Invoice() {
	...
	return (
		<Card size="2" style={{ maxWidth: 420 }}>
			<Typography.Heading size="3">Value codecs</Typography.Heading>
			<Typography.Text size="2" color="gray" style={{ display: "block", marginBottom: 12 }}>
				{price.format()} · {sku.format()}
			</Typography.Text>

			<DataTable.Root>
				{lines.map((line) => (
					<DataTable.Item key={line.sku.value}>
						<DataTable.Label>{line.label}</DataTable.Label>
						<DataTable.Value>
				...
				<DataTable.Item>
					<DataTable.Label>Total (minor units, mixed currency)</DataTable.Label>
					<DataTable.Value>{total}</DataTable.Value>
				</DataTable.Item>
			</DataTable.Root>
		</Card>
	);
